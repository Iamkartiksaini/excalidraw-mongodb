...
interface CreateLiveButtonProps {
  asCard?: boolean;
}

export default function CreateLiveButton({ asCard = false }: CreateLiveButtonProps) {
  ...
  const handleCreate = () => {
    startTransition(async () => {
      try {
        ...
        router.push(`/draw/live/${drawing._id}`);
      } catch (error) {
  ...
  if (asCard) {
    return (
      <button
        ...
        disabled={isPending}
        className="group h-full bg-white border-2 border-dashed border-[#e9ecef] hover:border-[#6965db] transition-all overflow-hidden flex flex-col items-center justify-center cursor-pointer disabled:opacity-50"
